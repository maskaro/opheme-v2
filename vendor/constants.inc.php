<?php
	...
	/**
	 * Controls system registration status.
	 */
	 ...
	 * Public location
	 */
	define('__oPUBLIC__', __oDIR__ . '/public');
	...
	elseif (substr_count($sn, 'ci_') > 0) {
		/**
		 * Company Identifier
		 */
		define('__oCompanyID__', 'opheme');
		/**
		 * Continuous Integration flag
		 */
		define('__oCI__', true);
	...
	elseif (substr_count($sn, '_') > 0) {
		/**
		 * Company Identifier
		 */
		define('__oCompanyID__', substr($sn, 0, stripos($sn, '_')));
		/**
		 * Continuous Integration flag
		 */
		define('__oCI__', false);
		 ...
		 * System Demo flag
		 */
		define('__oDEMO__', true);
	}
	elseif (is_numeric(strpos($sn, 'app.'))) {
		/**
		 * Company Identifier
		 */
		define('__oCompanyID__', 'opheme_app');
		/**
		 * oPheme App
		 */
		define('__oApp__', true);
		/**
		 * Continuous Integration flag
		 */
		define('__oCI__', false);
		 ...
		 * Company Identifier
		 */
		define('__oCompanyID__', getCompanyID($sn));
		/**
		 * Continuous Integration flag
		 */
		define('__oCI__', false);
	...
	// make sure this constant is always defined
	if (!defined('__oApp__')) { define('__oApp__', false); }
	 ...
	 * Company Brand URL
	 */
	define('__oCompanyBrandURL__', 'http' . (__oSSL_ENABLED__ === true?'s':'') . '://' . (__oCI__ === true?'ci_':'') . (__oDEMO__ === true?__oCompanyID__ . '_p':(__oApp__ === true?'app':'portal')) . '.' . __oCompanyDomain__);
	...
	/*
	 * System OAUTH Callback link
	 */
	define('__oOAUTH_CALLBACK__', __oCompanyBrandURL__ . '/callback');