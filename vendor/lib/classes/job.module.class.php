...
class Job
{
	...
	public function setStatus ( $details, $ajax = false )
	{
		...
		if ( !empty( $details[ 'jobType' ] ) )
		{
			...
			$data = array(
				...
				'type'		 => 'mysql',
				'operation'	 => 'update',
				'table'		 => 'app.' . $details[ 'jobType' ] . '.jobs' )
			;
	 ...
	 * @param string $messageType Social media module.
	 * @param string $id Message ID.
	 * @return boolean TRUE if it already exists, FALSE otherwise.
	 */
	public function messageExists ( $messageType, $id )
	{
	 ...
	 */
	public function getJobMessageCount ( $jobType, $jobId )
	{
	 ...
	 * @return boolean TRUE if successful, FALSE otherwise.
	 */
	public function storeMessage ( $smType, $message, $jobId = null, $module = null, $isSent = null, $linkOnly = null )
	{
		...
		if ( !empty( $jobId ) && !empty( $module ) && $return )
		{
		...
		if ( isset( $details[ 'id' ], $details[ 'jobType' ] ) )
		{
			...
			if ( is_array( $jobMessages ) )
			{
				...
				foreach ( $jobMessages as $jobMessage )
				{
					...
					//klout access
					if ( $this->company->hasAccessTo( 'klout' ) )
					{
						$klout					 = true;
						$fetch[ 'columns' ][]	 = 'klout';
					}
	...
	public function getJobTokens ( $jobType, $jobId, $forUi = false )
	{
		...
		if ( $result )
		{
			...
			{
				$token = $this->{$tokenInfo[ 'token_type' ]}->getUserTokens( 'one', $tokenInfo[ 'token_id' ], $forUi );
				if ( count( $token ) )
				{
