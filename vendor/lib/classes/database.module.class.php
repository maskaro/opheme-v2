<?php
	...
	class Database {
		...
		private $mysqlOpt = array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET CHARACTER SET utf8', PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8');
		...
		public function __construct(Array &$my_db, Array &$mo_db) {
			...
			try {
				...
				$this->mysqlConn = new PDO("mysql:host=" . $my_db['db.connection.host'] . ";dbname=" . $my_db['db.connection.db'] . ";charset=utf8", $my_db['db.connection.user'], $my_db['db.connection.pass'], $this->mysqlOpt);
				...
				$this->mongoConn = new MongoClient();
		...
		public function setErrorMessage($ex) {
			...
			if ($this->debug) { printf('Database Exception: %s', $errorMessage); }
			if (__oDEBUG_BACKEND__) { trigger_error('Database Exception: ' . $errorMessage . '.'); }
		 ...
		 *	&#09;'id', //No value needed. //Only for MySQL. //Optionally returns the ID of the INSERT operation.<br>
		...
		public function query($request = null) {
			...
			if ($this->debug) { printf('<br>Database Request: <pre>%s</pre>', print_r($this->request, true)); }
		...
		public function queryMySQLCustom($request) {
			...
			if ($this->debug) { printf('<br>Database Request: %s', print_r($request, true)); }
			...
			if ($this->debug) {
				...
				printf('<br>MySQL Query: <pre>%s</pre>', print_r($request['query'], true));
				printf('<br>MySQL Query Result: <pre>%s</pre>', print_r($this->mysqlResults['custom'], true));

				$timeTaken = calcMicroTimeDiff($queryStart, $queryEnd, 10);
				printf('<br>MySQL Query Execution Time: %s<br>', $timeTaken);
				...
				while ($query = array_pop($this->mysqlQueries)) {
					...
					$this->mysqlResults[$query['name']] = (
						...
						(
							...
							?
							intval($this->mysqlConn->lastInsertId())
							:
							$rowCount
						)
					);
					...
					if ($this->debug) {
						...
						printf('<br>MySQL Query: <pre>%s</pre>', print_r($query, true));
						printf('<br>MySQL Query Result: <pre>%s</pre>', print_r($this->mysqlResults[$query['name']], true));
				...
				while ($query = array_pop($this->mongoQueries)) {
					...
					try {
						...
						switch($query['type']) {
							...
							case 'insert':
								$this->mongoResults[$query['name']] = intval($coll->insert($query['data']));
								break;
							case 'update':
						...
						if ($this->debug) { 
							...
							printf('<br>MongoDB Query: %s', print_r($query, true));
							printf('<br>MongoDB Operation Status: %s', print_r($lastError, true));
							printf('<br>MongoDB Operation Result: %s', print_r($this->mongoResults[$query['name']], true));

							$timeTaken = calcMicroTimeDiff($queryStart, $queryEnd, 10);
							printf('<br>MongoDB Query Execution Time: %s<br>', $timeTaken);

						} else {
							if ($lastError['err']) { $this->setErrorMessage('<br>MongoDB Operation Error: ' . $lastError['err'] . '.'); }
						}
						
					} catch(MongoException $ex) {
						
						$errorMessage = 'Code: ' . $ex->getCode() . '. Message: ' . $ex->getMessage() . '.';
						$this->setErrorMessage($errorMessage);
