...
var reply_max_length = 140; //social media max message length
var share_max_length = 110; 
...
$("form#editor input").change(function() { $("form#editor").valid(); });
$("form#editor select").change(function() { $("form#editor").valid(); });
...
if (moduleId === "campaign") {
	...
	$("#response_text").keyup(function() {
		var chars = $(this).val();
		if (share_max_length - chars.length === -1) $("#response_text_count").html("0");
		else $("#response_text_count").html((share_max_length - chars.length));
		if (chars.length > share_max_length) {
			$(this).val(chars.substring(0, share_max_length));
		}
		...
		var preview = chars.replace("%r", "@Username").replace("%c", window["companyName"]);
