...
(function ($) {
	...
	$.oPhemeUI = function (el, options) {
		...
		plugin.clearMapElement = function () {
			plugin.internal._map_container.empty();
		};
		...
		plugin.map_gmaps_getMarkerIcon = function (info, self) {
			...
			date = new Date();
			created = Date.parse(info.created);
			...
			for (var threshold in ref.settings.ageThresholdOpacity) {
				if (ageMinutes < parseInt(threshold)) {
					opacity = ref.settings.ageThresholdOpacity[threshold];
					break;
				}
			}
			if (!opacity) {
				opacity = ref.settings.ageThresholdOpacity["old"];
			}
		...
		plugin.startQueue = function () {
			...
			if (plugin.internal._queue_timer === null) {
				...
				//set the timer
				plugin.internal._queue_timer = setInterval(function () {
					parseFunction(self);
		...
		plugin.pause = function () {
			...
			try {
				...
				data = false;
				plugin.displayMessage("danger", "Sorry, but the server decided to take a break. Please try again later.");
			}
			...
			if (messages instanceof Array && messages.length > 0) {
				...
				do {
					...
					try {
						...
						profileImg = "https:" + message.user.profile_image_url.substr(removeAmount, message.user.profile_image_url.length);
						timeAgo = message.created_at.substr(4, message.created_at.length).replace("BST ", "");

						smType = message.smType;
						smId = message.smId;

						screenName = "<a href='https://" + smType + ".com/" + message.user.screen_name + "' target='_blank'>@" + message.user.screen_name + "</a>";

						innerText = (smType === "instagram") ? (message._o_text ? message._o_text : message.text) : Autolinker.link(message.text, {className: "innerLink"});
								...
								<div class='opheme-bubble-content'>\
									...
									<div class='opheme-bubble-buttons'>\
										<button class='btn btn-primary btn-xs' onclick='if (confirm(\"Are you sure you want to follow @" + message.user.screen_name + "?\") === true) { oph_" + ref.settings.id + ".smFollow(\"" + smType + "\", \"" + smId + "\", \"" + message.user.id + "\", \"" + message.user.screen_name + "\"); }'>\
									  		<i class='fa fa-" + smType + "'></i> Follow\
										</button>\
										<button class='btn btn-primary btn-xs' onclick='createReplyForm(\"" + smType + "\", \"" + message.user.screen_name + "\", \"" + message.user.id + "\", \"" + message.id + "\", \"" + message.text + "\", \"" + message.created_at + "\")'>\
				...
				if ($("#loader-element.map-loader").is(":visible")) {
		...
		plugin.smFollow = function (smType, smId, user_id, screen_name) {
			...
			try {
				...
				if (data === true) {
					...
					status = "danger";
					message = "User has not been successfully followed. Reason: " + data + ". Please contact Support for assistance.";
				}
		...
		plugin.share = function () {
			...
			try {
				...
				if (data.status === true) {
					...
					status = "danger";
					message = "Job has not been successfully shared. Reason: " + data.status + ". Please contact Support for assistance.";
				}
		...
		plugin.sendReplyToSM = function (msg, smType, smId, usn, uId, mId, origMsg, origMsgDate) {

			var data;

			try {

				plugin.doThisASync("socialMedia/sendReply", {oUserId: plugin.settings.oUserId, smType: smType, smId: smId, message: msg, usn: usn, uId: uId, mId: mId, origMsg: origMsg, origMsgDate: origMsgDate});
		...
		plugin.doThisASync = function (action, data) {
		...
		plugin.isJsonString = function (str) {
			return (typeof ($.parseJSON(str)) === "object");
		};
