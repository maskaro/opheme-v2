...
Tooltip.prototype.draw = function() {
    ...
    // Position the DIV.
    this.div_.style.left = (ne.x + 25) + "px";
    this.div_.style.top = (ne.y + 25) + "px";
    ...
    if (this.div_) {
		if (!now) {
			this._to = setTimeout(function(el) {
				...
				window.clearTimeout(el._to);
				el._to = null;
			}, 2500, this);
			return this._to;
		} else {
			if (this._to) { window.clearTimeout(this._to); this._to = null; }
...
Tooltip.prototype.show = function() {
    ...
    if (me.div_) {
		...
		try {
			...
			//auto hide
			if (me._to) { window.clearTimeout(me._to); me._to = null; }
			me._to = me.hide();
			
			if ($(me.div_).is(":visible")) { return; }
